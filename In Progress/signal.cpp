...
//------------ Control ------------
int subrunLow=5, subrunHigh=10, subrunPoint = 8; //controls peak finding limits (subrun and channel) - need baseline info (mean and rms) - found from subrun 1
...
const char file2[100] = "20170816T145947_fembTest_gainenc_test_g3_s2_extpulse.root";
...
class Analyze {
    ...
    void set_pos_peak_risetime(int,int, std::vector<unsigned short> wf[64][128]);
...
void Analyze::set_baseline(int subrun, int channel, std::vector<unsigned short> wf[64][128]){
    TH1F *base_hist = new TH1F("base_hist1","",2000,1,5000); //histogram subrun 1 baseline #1
    for(int s = 0; s < wf[subrun][channel].size(); s++) base_hist->Fill(wf[subrun][channel].at(s));
    Double_t baseline1 = base_hist->GetMean(); //Get Baseline Mean
    cout << "Baseline (Subrun: " << subrun << "," << channel << "): " << baseline1 << endl;
    vector_struct[subrun][channel].at(0).baseline = baseline1;
}
...
void Analyze::set_pos_peak_risetime(int subrun, int channel, std::vector<unsigned short> wf[64][128]){
    Double_t ratio = vector_struct[subrun][channel].at(0).pos_peak_y.at(0)/wf[subrun][channel].at(vector_struct[subrun][channel].at(0).pos_peak_x.at(0));
    cout << "ratio test: " << ratio << endl;
    //vector_struct[subrun][channel].at(0).set_pos_peak_risetime = neg_rms->GetRMS();
}
...
void test(){
    ...
    TH1F *hb_1 = new TH1F("hb_1","",2000,1,5000); //histogram subrun 1 baseline #2
    ...
    c0->cd(1);
    int subrun = 1;
    sprintf(titleGraph,"Waveform (Subrun: %d, Channel %d)",subrun,channelPoint);
    for(int s = 0; s < wf_root1[subrun][channelPoint].size(); s++) h1_1->SetBinContent(s , wf_root1[subrun][channelPoint].at(s)); //draw input waveform
    for(int s = 0; s < wf_root2[subrun][channelPoint].size(); s++) h1_2->SetBinContent(s , wf_root2[subrun][channelPoint].at(s)); //draw input waveform
    ...
    subrun = 2;
    sprintf(titleGraph,"Waveform (Subrun: %d, Channel %d)",subrun,channelPoint);
    for(int s = 0; s < wf_root1[subrun][channelPoint].size(); s++) h2_1->SetBinContent(s , wf_root1[subrun][channelPoint].at(s)); //draw input waveform
    for(int s = 0; s < wf_root2[subrun][channelPoint].size(); s++) h2_2->SetBinContent(s , wf_root2[subrun][channelPoint].at(s)); //draw input waveform
    ...
    subrun = 3;
    sprintf(titleGraph,"Waveform (Subrun: %d, Channel %d)",subrun,channelPoint);
    for(int s = 0; s < wf_root1[subrun][channelPoint].size(); s++) h3_1->SetBinContent(s , wf_root1[subrun][channelPoint].at(s)); //draw input waveform
    for(int s = 0; s < wf_root2[subrun][channelPoint].size(); s++) h3_2->SetBinContent(s , wf_root2[subrun][channelPoint].at(s)); //draw input waveform
    ...
    subrun = 4;
    sprintf(titleGraph,"Waveform (Subrun: %d, Channel %d)",subrun,channelPoint);
    for(int s = 0; s < wf_root1[subrun][channelPoint].size(); s++) h4_1->SetBinContent(s , wf_root1[subrun][channelPoint].at(s)); //draw input waveform
    for(int s = 0; s < wf_root2[subrun][channelPoint].size(); s++) h4_2->SetBinContent(s , wf_root2[subrun][channelPoint].at(s)); //draw input waveform
    ...
    subrun = 5;
    sprintf(titleGraph,"Waveform (Subrun: %d, Channel %d)",subrun,channelPoint);
    for(int s = 0; s < wf_root1[subrun][channelPoint].size(); s++) h5_1->SetBinContent(s , wf_root1[subrun][channelPoint].at(s)); //draw input waveform
    for(int s = 0; s < wf_root2[subrun][channelPoint].size(); s++) h5_2->SetBinContent(s , wf_root2[subrun][channelPoint].at(s)); //draw input waveform
    ...
    subrun = 6;
    sprintf(titleGraph,"Waveform (Subrun: %d, Channel %d)",subrun,channelPoint);
    for(int s = 0; s < wf_root1[subrun][channelPoint].size(); s++) h6_1->SetBinContent(s , wf_root1[subrun][channelPoint].at(s)); //draw input waveform
    for(int s = 0; s < wf_root2[subrun][channelPoint].size(); s++) h6_2->SetBinContent(s , wf_root2[subrun][channelPoint].at(s)); //draw input waveform
    ...
    subrun = 7;
    sprintf(titleGraph,"Waveform (Subrun: %d, Channel %d)",subrun,channelPoint);
    for(int s = 0; s < wf_root1[subrun][channelPoint].size(); s++) h7_1->SetBinContent(s , wf_root1[subrun][channelPoint].at(s)); //draw input waveform
    for(int s = 0; s < wf_root2[subrun][channelPoint].size(); s++) h7_2->SetBinContent(s , wf_root2[subrun][channelPoint].at(s)); //draw input waveform
    ...
    subrun = 8;
    sprintf(titleGraph,"Waveform (Subrun: %d, Channel %d)",subrun,channelPoint);
    for(int s = 0; s < wf_root1[subrun][channelPoint].size(); s++) h8_1->SetBinContent(s , wf_root1[subrun][channelPoint].at(s)); //draw input waveform
    for(int s = 0; s < wf_root1[subrun][channelPoint].size(); s++) h8_2->SetBinContent(s , wf_root2[subrun][channelPoint].at(s)); //draw input waveform
    ...
    subrun = 9;
    sprintf(titleGraph,"Waveform (Subrun: %d, Channel %d)",subrun,channelPoint);
    for(int s = 0; s < wf_root1[subrun][channelPoint].size(); s++) h9_1->SetBinContent(s , wf_root1[subrun][channelPoint].at(s)); //draw input waveform
    for(int s = 0; s < wf_root2[subrun][channelPoint].size(); s++) h9_2->SetBinContent(s , wf_root2[subrun][channelPoint].at(s)); //draw input waveform
    ...
    subrun = 10;
    sprintf(titleGraph,"Waveform (Subrun: %d, Channel %d)",subrun,channelPoint);
    for(int s = 0; s < wf_root1[subrun][channelPoint].size(); s++) h10_1->SetBinContent(s , wf_root1[subrun][channelPoint].at(s)); //draw input waveform
    for(int s = 0; s < wf_root2[subrun][channelPoint].size(); s++) h10_2->SetBinContent(s , wf_root2[subrun][channelPoint].at(s)); //draw input waveform
    ...
    //------------------------------------------------------------------------------------- get baseline
    //----   1. ADC Counts for wf#1 & wf#2 Baseline Comparison - get baseline..
    //------------------------------------------------------------------------------------- get baseline
    ...
    for (int s=subrunLow; s <(subrunHigh+1); s++) { //+1 to reach max iterator
        for (int c=channelLow; c <(channelHigh+1); c++) { //+1 to reach max iterator
            ...
            foo.set_peaks(s, c, wf_root1, baseline1, 400); // store x-positions - 497 interval
    ...
    //foo.set_baseline(1,127,wf_root1);
    //foo.set_pos_peak_risetime(subrunPoint, channelPoint, wf_root1);
    ...
    /*TCanvas *c4 = new TCanvas("c4", "c4", 100, 10, 1000, 500);
    TH1F *wfH_test = new TH1F("","ADC Signal",1201,0,1200);
    TSpectrum *s2 = new TSpectrum(4);
    for(int s = 600; s < 1201; s++) wfH_test->SetBinContent(s , wf_root1[subrunPoint][channelPoint].at(s)); //draw input waveform
    Int_t nfound1 = s2->Search(wfH_test, 4, "", .25); //get peaks to get interval...
    Double_t *xpeaks1 = s2->GetPositionX();  //get x-position
